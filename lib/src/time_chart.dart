...
class _TimeChartState extends State<TimeChart>
  ...
  LinkedScrollControllerGroup _scrollControllers = LinkedScrollControllerGroup();
  ...
  void _tooltipCallback({
    ...
    if((_tooltipHideTimer?.isActive ?? false) && _currentVisibleTooltipRect == rect)
      return;
  ...
  Widget _buildOverlay(Rect rect, ScrollPosition position, double barWidth, {
    DateTimeRange range,
    double amount,
    DateTime amountDate,
  }) {
    final chartType = amount == null
        ? ChartType.time
        : ChartType.amount;
    // 현재 위젯의 위치를 얻는다.
    ...
    final candidateTop = rect.top + pivotOffset.dy - tooltipSize.height / 2 + _topPadding
  ...
  double _getRightMargin(BuildContext context) {
    ...
    final TextPainter tp = TextPainter(
      text: TextSpan(
        ...
        style:
        Theme.of(context).textTheme.bodyText2.copyWith(color: Colors.white38),
      ),
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (widget.chartType == ChartType.amount)
      return false;

    if (notification is ScrollStartNotification) {
      _cancelTimer();
    } else if (notification is ScrollEndNotification) {
      _updatePivotHourTimer = Timer(const Duration(milliseconds: 800), _timerCallback);
    }
    return true;
  }
  ...
  void _timerCallback() {
    ...
    final block = getCurrentBlockIndex(_barController.position, _blockWidth).toInt();
    ...
    if(topHour == beforeTopHour && bottomHour == beforeBottomHour)
      return;
  ...
  void _heightAnimation(int beforeTopHour, int beforeBottomHour) {
    ...
    setState(() {
      _beginHeight = (currentDiff / beforeDiff) * heightWithoutLabel + kXLabelHeight;
  ...
  Widget build(BuildContext context) {
    ...
    final double width = widget.width ?? MediaQuery.of(context).size.width - 32.0;
